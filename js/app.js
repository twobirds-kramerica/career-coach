...
function getActiveApiKey() {
  try {
    return ccGetStoredApiKey('llm_api_key')
        || ccGetStoredApiKey('cc_api_key')
        || '';
  } catch(e) { return ''; }
}
...
function viewOriginalPosting(jobId) {
  ...
  if (!w) return;
  // DOM-built instead of document.write(); textContent escapes automatically.
  const pre = w.document.createElement('pre');
  pre.style.cssText = 'font-family:sans-serif;padding:20px;white-space:pre-wrap';
  pre.textContent = job.rawPosting;
...
// Opens `html` (a full document string) in a new window without document.write()
// — navigates the window to a Blob URL instead. Same rendered output, avoids the
// document.write() XSS/performance footgun. Caller may attach a window 'load'
// listener before the window navigates if it needs to act once content is ready.
...
<style>
  ...
  const w = openHtmlInNewWindow(html);
